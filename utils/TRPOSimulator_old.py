...
class Simulator:
    def __init__(self, env, policy, n_trajectories, trajectory_len, delay=0, stochastic_delays=False, seed=None, **env_args):
        self.env = np.asarray([NormalizedEnv(env(delay=delay, stochastic_delays=stochastic_delays)) for _ in range(n_trajectories)])
...
class SinglePathSimulator(Simulator):
    def __init__(self, env_name, policy, n_trajectories, trajectory_len, delay=0, **env_args):
    ...
    def sample_trajectories(self):
        ...
        with torch.no_grad():
            ...
            while not np.all(done):
                ...
                policy_input = torch.stack([trajectory['states'][-1].clone().detach().to(self.device)
        ...
        with torch.no_grad():
            ...
            while not np.all(done):
                ...
                for env, action, trajectory in zip(continuing_envs, actions, trajs_to_update):
                    ...
                    # trajectory['hidden_states'].append(torch.tensor(env.hidden_obs).float())
                    if info[0] > 0:
                        trajectory['states'].append(torch.tensor(info[1]).float())
